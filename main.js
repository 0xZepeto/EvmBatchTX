...
// Fungsi untuk mengirim native token dengan retry yang ditingkatkan (diperbaiki)
async function sendNativeWithRetry(wallet, to, amount, explorer, symbol, customGasPrice, maxRetries = 5) {
  let nonce = await wallet.provider.getTransactionCount(wallet.address, "pending");
  
  // Gunakan gas price yang dipilih user
  let gasPrice = customGasPrice || await wallet.provider.getFeeData().then(fd => fd.gasPrice);
  ...
  try {
    ...
    gasLimit = ethers.toBigInt(21000); // Default lebih rendah
  }
  
  const gasFee = gasPrice * gasLimit;
  let totalNeeded = amount + gasFee;
  ...
  if (balance < totalNeeded) {
    ...
    console.log(chalk.yellow(`⚠️ Menyesuaikan jumlah pengiriman dari ${ethers.formatEther(amount)} ${symbol} menjadi ${ethers.formatEther(newAmount)} ${symbol}`));
    amount = newAmount;
    totalNeeded = amount + gasFee;
  }
    ...
    try {
      ...
      if (error.message.includes('nonce too low')) {
        ...
        if (currentBalance > currentGasFee) {
          ...
          console.log(chalk.yellow(`⚠️ Menyesuaikan jumlah pengiriman menjadi ${ethers.formatEther(newAmount)} ${symbol}`));
          amount = newAmount;
        } else {
...
async function sendTokenWithRetry(wallet, tokenContract, to, amount, explorer, symbol, customGasPrice, maxRetries = 5) {
  let nonce = await wallet.provider.getTransactionCount(wallet.address, "pending");
  ...
  for (let i = 0; i < maxRetries; i++) {
    ...
    try {
      // Buat transaksi secara manual untuk memastikan semua parameter benar
      const tx = {
        to: await tokenContract.getAddress(),
        data: tokenContract.interface.encodeFunctionData('transfer', [to, amount]),
        nonce: nonce,
        gasPrice: gasPrice,
        gasLimit: 0 // Akan diisi setelah estimasi
      };
      ...
      console.log(chalk.gray(`📊 Gas Limit: ${gasLimit.toString()}, Gas Price: ${ethers.formatUnits(gasPrice, 'gwei')} Gwei`));
      console.log(chalk.gray(`💰 Amount: ${ethers.formatUnits(amount, 18)} ${symbol}`));
      ...
      if (error.message.includes('nonce too low')) {
        ...
        log.error(`Saldo tidak cukup untuk transaksi`);
        throw error;
      } else if (error.message.includes('transfer from the zero address')) {
        log.error(`Error kontrak token: transfer from the zero address`);
        log.warning(`Mencoba dengan pendekatan alternatif... (${i + 1}/${maxRetries})`);
        
        // Coba dengan pendekatan alternatif: panggil fungsi transfer langsung
        try {
          ...
          tx.gasLimit = gasLimit * ethers.toBigInt(120) / ethers.toBigInt(100);
          
          const txResponse = await wallet.sendTransaction(tx);
          spinner.succeed(`✅ Transaksi terkirim (alternatif): ${txResponse.hash}`);
          log.tx(`HASH: ${txResponse.hash}`);
          log.explorer(txResponse.hash, explorer);
          return txResponse;
        } catch (altError) {
          log.error(`Pendekatan alternatif gagal: ${altError.message}`);
          if (i === maxRetries - 1) throw error;
        }
      } else if (error.message.includes('intrinsic gas')) {
        log.warning(`Masalah gas limit, menaikkan gas limit... (${i + 1}/${maxRetries})`);
    ...
    const batchPromises = batch.map(async (pk, index) => {
      try {
        ...
        try {
          ...
          // Tambahkan buffer 20% untuk keamanan
          gasLimit = estimatedGas * ethers.toBigInt(120) / ethers.toBigInt(100);
        } catch (error) {
        ...
        if (amountOption === 'all') {
          ...
          // Tidak ada batasan minimal saldo
        } else {
...
async function main() {
  try {
    ...
    // Pilih kecepatan transaksi
    const gasPrice = await selectGasSpeed();
    console.log(chalk.white(`⚡ Gas Price: ${ethers.formatUnits(gasPrice, 'gwei')} Gwei`));
    ...
    if (mainOption === 'token') {
      ...
      try {
        ...
        log.header(`🪙 TOKEN: ${tokenName} (${tokenSymbol})`);
        console.log(chalk.white(`📊 Decimals: ${tokenDecimals}`));
      } catch (error) {
        throw new Error(`Gagal terhubung ke kontrak token: ${error.message}`);
      }
      ...
      if (tokenOption === 'multi') {
        ...
        if (confirm) {
          ...
          for (let i = 0; i < transactions.length; i++) {
            ...
            try {
              await sendTokenWithRetry(wallet, tokenContract, tx.to, tx.amount, explorer, tokenSymbol, gasPrice);
      ...
      if (nativeOption === 'multi') {
        ...
        if (amountOption === 'fixed') {
          ...
          const balance = await provider.getBalance(senderAddress);
          // Kurangi biaya gas
          const gasFee = gasPrice * ethers.toBigInt(21000); // Gunakan gas limit dengan buffer
        ...
        // Cek saldo
        const balance = await provider.getBalance(senderAddress);
        const estimatedGasCost = gasPrice * ethers.toBigInt(21000) * ethers.toBigInt(recipients.length);
        ...
        if (confirm) {
          ...
          for (let i = 0; i < transactions.length; i++) {
            ...
            try {
              await sendNativeWithRetry(wallet, tx.to, tx.amount, explorer, symbol, gasPrice);
